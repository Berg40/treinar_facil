...
function calcularTMB() {
    ...
    // ==========================================
    // 🚀 ATIVAÇÃO DO BOTÃO DE DIETA E SAVE
    // ==========================================
    
    // 1. Salva dados no LocalStorage para a página de dieta usar
    const dadosUsuario = {
        tmb: tmbFinal,
        get: get,
        peso: peso,
    ...
    localStorage.setItem("dadosUsuarioFitness", JSON.stringify(dadosUsuario));

    // 2. Torna o botão visível
    const boxDieta = document.getElementById("boxDieta");
    if(boxDieta) {
        boxDieta.style.display = "block"; // Aqui a mágica acontece
        
        // Scroll suave até o botão para o usuário ver
        setTimeout(() => {
            boxDieta.scrollIntoView({ behavior: 'smooth', block: 'center' });
        }, 500);
    }
}
